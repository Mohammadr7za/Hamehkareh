...
<div class="d-flex justify-content-end align-items-center">
    @if(!$user->trashed())
        <a class="mr-2" href="{{ route('user.getchangepassword',['id' => $user->id]) }}"
           title="{{ __('messages.change_password',['form' => __('messages.user') ]) }}"><i
                class="fa fa-lock text-success "></i></a>
        ...
        @if($auth_user->can('user edit'))
            <a class="mr-2" href="{{ route('user.create',['id' => $user->id]) }}"
        ...
        @endif
    @endif
    @if(auth()->user()->hasAnyRole(['admin' ,'manager']) && $user->trashed())
